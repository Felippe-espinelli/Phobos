...
- When quantification is finished, the program will automatically generate an MS-Excel file (.xls) with the results. For each video, the software will display the video duration, total freezing (in sec and %) and freezing time for specific time bins. To create another .xls file using another length for the time bins, follow step 5.
